...
# ============================================================
#
# What each section of the seed command does and why it was written that way
#
# The imports bring in date helpers, Decimal, Django's user model helper, the
# BaseCommand class, timezone support, and the project models that need demo
# records.
#
# Command is the Django management command class. Django finds it because the
# file is named seed_data.py inside management/commands.
#
# The class-level demo lists store the known demo emails, category names, menu
# item names, inventory names, and table numbers. clear_demo_data uses these
# lists so it deletes only records owned by this demo seed command.
#
# handle is the main entry point. It clears old demo data, seeds each section
# in the requested order, prints a summary, and then prints a success message.
#
# clear_demo_data removes old demo orders, reservations, menu links, menu
# items, categories, inventory items, tables, and demo users. It does not
# delete superusers, even if a superuser uses one of the demo email addresses.
#
# seed_users creates one manager, two staff users, and three customers. It
# sets passwords every time so the demo credentials stay predictable.
#
# seed_categories creates the four menu sections in display order.
#
# seed_menu_items creates at least two available menu items per category with
# realistic prices and preparation times.
#
# seed_inventory_items creates stock records. Potatoes are seeded below their
# reorder level so the low-stock demo is visible immediately.
#
# seed_menu_item_ingredients links menu items to inventory items so order
# creation can deduct stock through the existing signals.
#
# seed_tables creates eight active tables across indoor, outdoor, and private
# locations with the requested capacities.
#
# seed_reservations creates three demo bookings for today and tomorrow, with
# party sizes that fit the selected tables.
#
# seed_orders creates three served orders for the daily sales report plus one
# preparing order and one received order. Each order has two or three items.
#
# make_datetime creates timezone-aware reservation datetimes.
#
# print_summary prints how many records were created for each model.
#
# Why orders must be seeded last
#
# Orders depend on customers, tables, menu items, and menu item ingredients.
# The order item signal also uses recipe links to deduct inventory. Seeding
# orders last makes sure every required object already exists before stock is
# deducted and totals are calculated.
#
# Why get_or_create is used instead of bulk_create
#
# get_or_create avoids duplicate errors when the command is run again or when
# a record already exists. It also works with model save logic, password
# hashing, and relationship setup in a clearer way than bulk_create.
#
# bulk_create skips some per-object behavior and is harder to use safely when
# records need unique fields, updates, passwords, and related objects.
#
# What I should check after running the command
#
# Check that the demo users can log in with the listed credentials.
#
# Check that the menu endpoint shows eight available items across four
# categories.
#
# Check that inventory includes at least one low-stock item.
#
# Check that the daily sales report shows totals from the three served orders.
#
# Check that creating a new order deducts inventory through the signals.
#
# ============================================================
# END OF REVIEW
# ============================================================
